...
from models import Dt, Emp, Pj, EmpPj, db
from app import app
...
dl = Dt(dept_code='legal', dept_name='Legal', phone='555-2222')
dm = Dt(dept_code='mktg', dept_name='Marketing', phone='555-9999')

leonard = Emp(name='Leonard', dept=dl)
liz = Emp(name='Liz', dept=dl)
maggie = Emp(name='Maggie', state='DC', dept=dm)
nadine = Emp(name='Nadine')

db.session.add_all([df, dl, dm, leonard, liz, maggie, nadine])
db.session.commit()

pc = Pj(proj_code='car', proj_name='Design Car',
        assignments=[EmpPj(emp_id=liz.id, role='Chair'),
                     EmpPj(emp_id=maggie.id)])
ps = Pj(proj_code='server', proj_name='Deploy Server',
        assignments=[EmpPj(emp_id=liz.id),
                     EmpPj(emp_id=leonard.id, role='Auditor')])
